...
- **Scratch buffer protocol for trace replay**: The replay adapter allocates a single WASM memory page (64KB) using `memory.grow(1)` once, then reuses it across all ecalli calls as a persistent scratch buffer. The outer handler writes ecalli responses (new registers + memwrite data) to this scratch page at the PVM address obtained via `pvm_ptr`. The buffer is reused to avoid repeated allocations while working around the limitation that adapter globals are not supported by `adapter_merge`.
