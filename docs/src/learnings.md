...
2. **`Freeze` listed in `instruction_produces_value`** (`llvm_backend/emitter.rs`). The pre-scan walks every block and allocates a stack slot for any instruction whose result is consumed downstream; without `Freeze` in the producer set, `result_slot()` later panics with `no slot for Freeze result`. Easy to miss: the `lower_instruction` arm compiles cleanly without it and the passthrough is well-defined — the failure only surfaces when the test actually runs.
