...
- **Globals only occupy the bytes they actually need**: the compiler tracks `globals_region_size = (num_globals + (1 if memory.size/grow/init used else 0) + num_passive_segments) * 4` bytes and places the heap immediately after that region instead of reserving a full 64KB block. The compiler-managed memory-size slot is elided for programs that never read or grow memory size, saving 4 bytes of `rw_data`.
