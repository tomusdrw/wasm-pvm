...
### Multi-Predecessor Cross-Block Cache Propagation — Investigated, Zero Realized Impact (2026-05)

- **Idea**: Extend the single-predecessor cross-block cache forwarding to multi-predecessor blocks (forward merges, loop headers). At each merge, intersect all predecessor snapshots: `slot_cache[S] = R` survives only if every predecessor agrees, and similarly for `reg_to_const` and `alloc_reg_slot`. Invalidate phi-destination slots/registers in the merged snapshot (the snapshot was captured before each predecessor's terminator wrote phi copies). The expected upside was ~87 K cross-block redundant `LoadIndU64`s eliminated on polkadot runtimes (prior analysis: ~7 % of code).
- **Implementation**: Added `CacheSnapshot::intersect_with(&Self)` (rebuild `slot_cache`/`reg_to_slot` from the agreement set, intersect arrays element-wise) and `CacheSnapshot::invalidate_slot(i32)`. Unified single-pred and multi-pred propagation into one path gated on "every predecessor processed." Built `pred_map` whenever `use_cross_block_cache` is on. Made `set_alloc_reg_for_slot` lazy-spill-aware so `restore_phi_alloc_reg_slots` is safe in both spill modes. All four test suites green (670 integration, 142 differential, 277 PVM-in-PVM, full cargo).
- **Polkadot measurement, both runtimes**: byte-for-byte identical to baseline. glutton-kusama: 4,636,361 / 1,206,055 / 6,444,121 (code / instr / JAM) — Δ = 0. kusama: 17,965,423 / 4,604,992 / 22,217,687 — Δ = 0. Also zero with `--no-register-alloc` (rules out a peephole-absorbed delta).
- **Root cause 1 — `load_operand` skips `slot_cache` for allocated values**: The cache lookup is in the `else if let Some(slot)` arm, *after* the `regalloc.val_to_reg` check. With aggressive regalloc on polkadot, virtually every live value is allocated, so reads route through `alloc_reg_slot` and never look at `slot_cache`. Multi-pred `slot_cache` propagation helps a code shape that barely exists in regalloc'd runtimes.
- **Root cause 2 — the `alloc_reg_slot` intersection was already there**: The pre-existing `all_processed` branch (now in `lower_function`'s back-edge fallback) already computed `set_alloc_reg_slot_from(pred0) + intersect_alloc_reg_slot(rest)`. `restore_cache(&merged)` ends up writing the same `alloc_reg_slot` values. End-state alloc tracking is unchanged.
- **Root cause 3 — block layout prevents `all_processed` at most multi-pred merges**: `compute_block_layout` is greedy fallthrough-biased, not topological. A canonical if-else (`entry → {then, else} → join`) lays out as `entry, else, join, then` — at `join`'s emission time `then` is unprocessed, so `all_processed=false` and the propagation skips. Instrumented count on glutton-kusama: 48,473 candidate merges, only 3,502 genuinely multi-pred fires (rest are single-pred), of which 84 % had a predecessor with already-empty `slot_cache` (calls clear it), leaving ~250 propagated entries function-wide.
- **What would actually move the needle**: (a) emit blocks in RPO so `all_processed` fires at every forward merge (sacrifices fallthrough opt), (b) loop-body register-liveness analysis to safely propagate `alloc_reg_slot` through back-edges, or (c) a two-pass scheme that computes per-block exit snapshots in a dataflow pre-pass then emits with them available. All three are substantially larger than a localized propagation tweak.
- **Conclusion**: Investigated and verified the change is correct — but the optimization it enables doesn't apply to regalloc'd polkadot workloads, and even where it does apply (non-allocated values), block emission order prevents firing at the merges where the wins would live. Not landed.
