...
async function main() {
  ...
  // roLength (3 bytes, little-endian)
  ...
  // Show first few jump table entries
  ...
  // Count instructions (1 bits in mask)
  let instrCount = 0;
  ...
  // Verify we consumed approximately the right amount
  ...
  // Try to load with anan-as
  console.log();
  console.log('=== anan-as Validation ===');
  try {
    const ananAs = await import(ananAsPath);
    
    // Try to prepare the program (will validate structure)
