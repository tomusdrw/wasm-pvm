      ...
      (then
        (local.set $result
          (block (result i32)
            ...
            (drop)          ;; Drop the 20 (unreachable but needed for validation)
            (i32.const 999) ;; Fallthrough value (won't execute)
          )
        )
      )
    )
        ...
        (local.set $result
          (block (result i32)  ;; Outer block
            ...
            (block (result i32)  ;; Inner block
              ...
              (drop)          ;; Drop the 40 (unreachable but needed for validation)
              (i32.const 50)  ;; Fallthrough value (won't execute)
            )
            (drop)  ;; Drop the inner block result (40 or 50)
          )
        )
      )
    )
