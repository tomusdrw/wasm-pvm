...
- **Peephole shift-pair cleanup**: `peephole.rs` removes redundant `ShloL64+ShloR64` i32-zext pairs when the value is already known zero-extended (e.g. `LoadU32`/`LoadIndU32`/`ZeroExtend16`/non-negative `LoadImm`). The matcher can look backward through non-clobbering instructions and `MoveReg` chains, but does not cross label targets.
